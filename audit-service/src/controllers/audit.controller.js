...
const kafka = new Kafka({ clientId: 'tenant-service', brokers: ['kafka:9092'] });
...
const createTenant = async (req, res) => {
  ...
  if (
    logoUrl &&
    !/^https?:\/\/(?:res\.cloudinary\.com\/[a-zA-Z0-9_-]+\/image\/upload\/.+|.+\.(jpg|jpeg|png|gif|svg))$/.test(logoUrl)
  ) {
    return res.status(400).json({
      error: 'Invalid logo URL. Must be a valid image URL (jpg, jpeg, png, gif, svg) or a Cloudinary URL.',
    });
  }
...
const createRole = async (req, res) => {
  ...
  try {
    ...
    await producer.send({
      topic: 'role.created',
      messages: [{ value: JSON.stringify({ id: role.id, name: role.name, tenantId }) }],
    });
...
const createUser = async (req, res) => {
  ...
  try {
    ...
    await producer.send({
      ...
      messages: [{ value: JSON.stringify({ id: user.id, email: user.email, roleId, tenantId }) }],
    });
    await producer.disconnect();
